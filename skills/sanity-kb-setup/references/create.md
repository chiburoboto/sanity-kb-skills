...
1. Run `npx sanity context list --organization <org-id>`. This proves the login works and shows which Knowledge Bases exist. If one already belongs to this project, reuse it. Read its imports with `npx sanity context imports list <kb-id>`, add only the sources that are missing, and skip to the build. If the organisation is at its limit, stop and open `blocked.md`. Don't delete one to make room unless the user names it.
